...
local RACE_CHECKPOINT_TYPE = 44
local RACE_CHECKPOINT_FINISH_TYPE = 26
            ...
            if raceStatus.state == RACE_STATE_RACING then
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if config_cl.checkpointRadius > 0 then
                        ...
                        checkpoint.checkpoint = CreateCheckpoint(checkpointType, checkpoint.coords.x,  checkpoint.coords.y, checkpoint.coords.z, 0, 0, 0, config_cl.checkpointRadius, 255, 255, 0, 127, raceStatus.checkpoint)
                    ...
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < config_cl.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            if config_cl.checkpointRadius > 0 then
                                local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                                nextCheckpoint.checkpoint = CreateCheckpoint(checkpointType, nextCheckpoint.coords.x,  nextCheckpoint.coords.y, nextCheckpoint.coords.z, 0, 0, 0, config_cl.checkpointRadius, 255, 255, 0, 127, raceStatus.checkpoint)
...
function Draw3DTextAdv(x, y, z, text, font, r , g , b, a)
    -- Check if coords are visible and get 2D screen coords
    local onScreen, _x, _y = World3dToScreen2d(x, y, z)
    if onScreen then
        -- Calculate text scale to use
        ...
        while true do
            ...
            for i, value in pairs(allBlips) do
                ...
                if distance < 50.0 then
                    ...
                    local temp, zCoord = GetGroundZFor_3dCoord(markerPos.x, markerPos.y, 9999.9, 1)
                    Draw3DTextAdv(markerPos.x, markerPos.y, zCoord+2.0, ("Race Event: " .. allBlips[i][1]), 1, 255 , 151 , 220, 255)
